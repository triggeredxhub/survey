...
import { fetchData, createChart } from './charts.js';

document.addEventListener('DOMContentLoaded', () => {
    const statistics = document.getElementById('statistics');
    const container = document.querySelector('.containerTesti');
    const filterContainer = document.createElement('div');
    filterContainer.classList.add('filterContainer', 'container', 'filters');
    filterContainer.style.display = 'none'; // Hide the filter container initially

    // Create and append dropdowns to the filterContainer with default values
    createDropdown(filterContainer, 'startYear', 'Start Year', [2024,2023, 2022, 2021], '2023');
    createDropdown(filterContainer, 'startMonth', 'Start Month', [
    ...
    createDropdown(filterContainer, 'endYear', 'End Year', [2024, 2023, 2022], '2024');
    createDropdown(filterContainer, 'endMonth', 'End Month', [
        { value: '01', text: 'January' },
        { value: '02', text: 'February' },
        { value: '03', text: 'March' },
        { value: '04', text: 'April' },
    ...
    // Append the filterContainer to the container but keep it hidden initially
    // container.appendChild(filterContainer);

    statistics.addEventListener('click', () => {
        // Show the filter container on click
        ...
        // Clear previous chart but keep the filterContainer
        container.innerHTML = '';
        container.appendChild(filterContainer);

        fetchData(startYear, startMonth, endYear, endMonth).then(data => createChart('.containerTesti', data));
    });

    function createDropdown(container, id, label, options, defaultValue) {
    ...
    const list = document.getElementById('listSurvey');
